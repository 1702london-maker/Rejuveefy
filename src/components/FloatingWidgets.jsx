...
export default function FloatingWidgets() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: "Hi! I'm Dora 👋 Rejuveefy's beauty assistant. I can help with bookings, hair care advice, product questions, and more. What can I help you with today?",
    }
  ])
  ...
  const resetChat = () => {
    setMessages([{
      role: 'assistant',
      content: "Hi! I'm Dora 👋 Rejuveefy's beauty assistant. How can I help you today?"
    }])
    setTransferring(false)
    setInput('')
  }
  ...
  return (
    <>
      {/* ── Right-side stacked floating buttons ──────────────── */}
      <div className="fixed bottom-24 right-5 z-50 flex flex-col items-center gap-3">
        {/* WhatsApp — icon only, on top */}
        <a
          href={WHATSAPP_URL}
          target="_blank"
          rel="noreferrer"
          className="w-14 h-14 bg-[#25D366] rounded-full flex items-center justify-center shadow-lg hover:bg-[#1ebe5a] transition-all hover:scale-105"
        ...
        {/* Chat bubble — below WhatsApp */}
        {!chatOpen && (
          <button
            onClick={() => setChatOpen(true)}
            className="w-14 h-14 bg-pink-500 rounded-full flex items-center justify-center shadow-lg hover:bg-pink-600 transition-all hover:scale-105 relative"
      ...
      {/* ── Chat window ──────────────────────────────────────── */}
      {chatOpen && (
        <div className={`fixed bottom-24 right-5 z-50 w-[360px] bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col transition-all duration-200 ${minimised ? 'h-14' : 'h-[520px]'}`}>
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-pink-500 to-rose-500 rounded-t-2xl text-white shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-sm font-bold leading-none">Dora</p>
